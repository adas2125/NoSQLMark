# Mambo mongod CPU experiment

Continuous-run variant of the August 29, 2026 mongod CPU experiment.
...
- Topology goal: `1 shard x 1 member -> 2 shards x 1 member -> 2 shards x 2 members`
...
- If the duplicate-membership error recurs but both secondaries are healthy,
  allow YCSB to continue so the post-scaling steady state is measured.
- Use `Ctrl-C` early only if MongoDB becomes unhealthy or the experiment must
  stop. The plotter can process a partial log.
- After YCSB finishes, `Ctrl-C` the Controller and Kubernetes terminals.
...
## 10. If duplicate-membership failure recurs

August 29 observation: data plane reached 2 x 2, but Mambo stayed in
`ScalingUpAddingReplica`. Bitnami had registered ordinal-1; Mambo's redundant
`rs.add()` failed with a duplicate-host error.
...
kubectl --kubeconfig /users/adas2125/.kube/amit.kubeconfig \
  ...
  -n mambo-mongod-cpu -o json | jq '.status'
kubectl --kubeconfig /users/adas2125/.kube/amit.kubeconfig \
  get statefulsets -n mambo-mongod-cpu
kubectl --kubeconfig /users/adas2125/.kube/amit.kubeconfig \
  get jobs -n mambo-mongod-cpu -o wide
kubectl --kubeconfig /users/adas2125/.kube/amit.kubeconfig \
  logs -n mambo-mongod-cpu \
  job/add-membership-my-mongodb-sharded-shard1-data-1 \
  --all-containers --tail=100

kubectl --kubeconfig /users/adas2125/.kube/amit.kubeconfig \
  exec -n mambo-mongod-cpu my-mongodb-sharded-shard0-data-0 -- bash -c '
  ...
  '
kubectl --kubeconfig /users/adas2125/.kube/amit.kubeconfig \
  exec -n mambo-mongod-cpu my-mongodb-sharded-shard1-data-0 -- bash -c '
    /opt/bitnami/mongodb/bin/mongosh --host 127.0.0.1 --port 27017 \
      -u root \
      -p "$(cat /bitnami/mongodb/secrets/mongodb-root-password)" \
      --authenticationDatabase admin --quiet \
...
Expected if the same failure recurs: one `PRIMARY` and one healthy `SECONDARY` per shard. If the controller reaches `Idle` without a failed membership Job, skip the failure snapshots and scaling-to-zero commands below.
...
kubectl --kubeconfig /users/adas2125/.kube/amit.kubeconfig \
  ...
  > "$RUN_DIR/mongodautoscaler-stuck-membership.yaml"
kubectl --kubeconfig /users/adas2125/.kube/amit.kubeconfig \
  get jobs -n mambo-mongod-cpu -o yaml \
  > "$RUN_DIR/membership-jobs-failed.yaml"
```

`[WARNING: CLUSTER CHANGE]` Pause Mambo after evidence capture. MongoDB stays
running.
...
August 29 reference: shard0 654,973 documents (65.49%, one chunk); shard1 345,027
documents (34.50%, three chunks); total 1,000,000.

## 12. Python plotter

Included script:
[`analysis/plot_mongod_cpu.py`](analysis/plot_mongod_cpu.py).
...
- Optional cutoff: `--end-time HH:MM:SS` (UTC, using the first sample's date)
...
cd /users/adas2125/Autoscaling
source experiment-artifacts/current-run-env.sh
python3 analysis/plot_mongod_cpu.py "$RUN_DIR"
```

To stop the plot at `03:15:00` UTC:

```bash
python3 analysis/plot_mongod_cpu.py "$RUN_DIR" --end-time 03:15:00
```
